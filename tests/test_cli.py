...
def test_mcp_no_subcommand_prints_setup_guidance(monkeypatch, capsys):
    """'mempalace mcp' with no subcommand still prints howto."""
    monkeypatch.setattr(sys, "argv", ["mempalace", "mcp"])
    main()
    captured = capsys.readouterr()
    assert "mempalace mcp run" in captured.out
...
def test_mcp_run_dispatches(monkeypatch):
...
def test_mcp_run_starts_server(monkeypatch):
    ...
    from mempalace.cli import cmd_mcp_run
